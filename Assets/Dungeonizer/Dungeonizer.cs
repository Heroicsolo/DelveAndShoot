...
namespace Dungeonizer { 
	...
	public class Dungeonizer : MonoBehaviour {
        ...
        public float wallPositionOffset = 0f;
        public GameObject doorPrefab;
        [Range(0f, 360f)] public float doorRotationOffset = 0f;
        public Vector3 doorPositionOffset;
        //public GameObject doorCorners;
        public GameObject corridorFloorPrefab;
		public GameObject corridorWallPrefab;
        [Range(0f, 360f)] public float corridorWallRotationOffset = 0f;
        public float corridorWallPositionOffset = 0f;
		public GameObject groundBlockPrefab;
        public float groundBlockPositionOffset = 0f;
        public GameObject groundCornerBlockPrefab;
        [Range(0f, 360f)] public float groundCornerRotationOffset = 0f;
				...
				for (var i = 0; i < room_count; i++) {
					...
					if (rooms.Count == 0) {
						...
						if (collision_count == 0) {
							branch = UnityEngine.Random.Range (5, 20); //complexity
						}
						room.branch = branch;
						...
						if (direction == "set") {
							string newRandomDirection = directions[UnityEngine.Random.Range(0, directions.Count)];
							direction = newRandomDirection;
							while (direction == oldDirection)
							{
								newRandomDirection = directions[UnityEngine.Random.Range(0, directions.Count)];
								direction = newRandomDirection;
							}
						}
						this.roomMarginTemp = UnityEngine.Random.Range(0, this.roomMargin - 1);
				...
				for (int i = 0; i < rooms.Count; i++) {
					...
					if (roomB != null) {
						...
						while ((pointB.x != pointA.x) || (pointB.y != pointA.y)) {						
							...
							if(horizontalCorridor){			
								currentTile = map.Find(item => (pointB.x == item.x && pointB.y == item.y && Dungeon.walls.Contains(item.type)));
								if(currentTile == null) {
									map.Add(createTile(108, pointB.x, pointB.y));
								}

								currentTile = map.Find(item => (pointB.x == item.x && pointB.y == item.y && Dungeon.walls.Contains(item.type)));
								if(currentTile == null)
								{
									map.Add(createTile(1010, pointB.x, pointB.y));
								}							
							}
							else {
								currentTile = map.Find(item => (pointB.x == item.x && pointB.y == item.y && Dungeon.walls.Contains(item.type)));
								if(currentTile == null) {
									map.Add(createTile(109, pointB.x, pointB.y));
								}
								currentTile = map.Find(item => (pointB.x == item.x && pointB.y == item.y && Dungeon.walls.Contains(item.type))); 
								if(currentTile == null) {
									map.Add(createTile(1011, pointB.x, pointB.y));
								}
							}
		...
		public void Generate()
		{
			...
			foreach(MapTile mapTile in Dungeon.map) {
				...
				if (Dungeon.walls.Contains(tile) || (Dungeon.corridor_walls.Contains(tile) && !dungeonizer.corridorWallPrefab )) {
					...
					if(!makeIt3d){ //wall rotation stuff here.
						...
						created_tile.transform.Rotate(Vector3.up  * (wallRotationOffset + 90 * (tile -2))); // 3D corner rotation
					}

                    created_tile.transform.position += wallPositionOffset * created_tile.transform.forward;

                    GameObject groundBlockInstance = GameObject.Instantiate(groundBlockPrefab, created_tile.transform.position, Quaternion.identity) as GameObject;
                    
					groundBlockInstance.transform.position -= groundBlockPositionOffset * created_tile.transform.forward;

                    groundBlockInstance.transform.parent = transform;
                }
					...
					if(!makeIt3d){ // wall rotation stuff here.
						...
						created_tile.transform.Rotate(Vector3.up  * (corridorWallRotationOffset + 90 * (tile -2)));
					}

                    created_tile.transform.position += corridorWallPositionOffset * created_tile.transform.forward;
                }
				...
				if (Dungeon.corners.Contains(tile)) {
					...
					if(cornerPrefabToUse){ //there was a bug in this line. A good man helped for fix. Dungeonizer community is awesome. 			
                        ...
                        GameObject cornerGroundInstance = GameObject.Instantiate(groundCornerBlockPrefab, created_tile.transform.position, Quaternion.identity) as GameObject;

                        cornerGroundInstance.transform.rotation = created_tile.transform.rotation;

                        cornerGroundInstance.transform.Rotate(Vector3.up * groundCornerRotationOffset);

                        cornerGroundInstance.transform.parent = transform;
                    }
				}
			...
			for (int i = 0; i < spawnedObjectLocations.Count; i++) {
				...
				int randomIndex = UnityEngine.Random.Range (i, spawnedObjectLocations.Count);
						...
						if (createHere){ //means dungeonizer found a suitable place to put object.
							...
							if (!makeIt3d){
								...
								else
								{
									...
									{
										spawnRotation = Quaternion.Euler(new Vector3(0, UnityEngine.Random.Range(0, 360), 0));
									}
									else
									{
										spawnRotation = Quaternion.Euler(new Vector3(0, UnityEngine.Random.Range(0,2) * 90, 0));
									}
								}
                            ...
                            if (objectToSpawn.spawnByWall)
                            {
                                if (spawnLocation.byWall)
                                {
                                    if (spawnLocation.wallLocation == "s")
                                    {
                                        newObject.transform.position += Vector3.forward * (objectToSpawn.offsetFix + wallPositionOffset);
                                    }
                                    else if (spawnLocation.wallLocation == "w")
                                    {
                                        newObject.transform.position += Vector3.right * (objectToSpawn.offsetFix + wallPositionOffset);
                                    }
                                    else if (spawnLocation.wallLocation == "n")
                                    {
                                        newObject.transform.position += Vector3.back * (objectToSpawn.offsetFix + wallPositionOffset);
                                    }
                                    else if (spawnLocation.wallLocation == "e")
                                    {
                                        newObject.transform.position += Vector3.left * (objectToSpawn.offsetFix + wallPositionOffset);
                                    }
                                }
							}
				...
				for (int i = 0; i < spawnedObjectLocations.Count; i++) {
					if (spawnedObjectLocations[i].asDoor > 0){
						...
						if(!makeIt3d){
							...
							newObject.transform.Rotate(Vector3.up  * (doorRotationOffset - 90 * ( spawnedObjectLocations[i].asDoor - 1)));
						}

                        newObject.transform.position += newObject.transform.TransformVector(doorPositionOffset);
